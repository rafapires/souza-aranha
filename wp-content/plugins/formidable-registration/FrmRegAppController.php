...
class FrmRegAppController{
    ...
    function __construct(){
        ...
        add_filter( 'the_content', array( __CLASS__, 'print_messages' ), 25 );
    ...
    */
    ...
    public static function register_actions($actions) {
        ...
        include_once(self::path() . '/models/FrmRegAction.php');
    ...
    public static function _usermeta_row($meta_name=false, $field_id=''){
        ...
        if(isset($_POST['form_id'])){
            global $wpdb;
            $frm_field = new FrmField();
            $fields = $frm_field->getAll($wpdb->prepare('fi.form_id=%d', $_POST['form_id']). " and fi.type not in ('divider', 'html', 'break', 'captcha')", ' ORDER BY field_order');
        }
        ...
        if ( FrmRegAppHelper::is_below_2() ) {
            ...
            $meta_key = $meta_name;
            include(self::path() .'/views/new_usermeta_row.php');
        }
        die();
    }
    ...
    public static function create_user_trigger($action, $entry, $form) {
        ...
        if ( isset($settings['reg_display_name']) && !empty($settings['reg_display_name']) ) {
            $user_meta['display_name'] = self::_generate_display_name($settings);
        }
        ...
        $frm_entry_meta->delete_entry_meta($entry->id, $user_field);
        $frm_entry_meta->add_entry_meta($entry->id, $user_field, '', $user_id);
        
        //remove password from database
        if ( isset($settings['reg_password']) && !empty($settings['reg_password']) ) {
            $frm_entry_meta->delete_entry_meta($entry->id, (int) $settings['reg_password']);
        }
        ...
        self::new_user_notification($user_id, $user_meta['user_pass'], $form, $entry->id, $settings);
	...
	public static function prevent_pending_login( $user, $password ) {
		...
		if ( in_array( 'pending', (array) $user->roles ) ) {
            ...
            if ( in_array( 'email', $moderate_type ) ) {
    			...
    			return new WP_Error( 'pending', __( '<strong>ERROR</strong>: You have not yet confirmed your e-mail address. <a href="' . $resend_link . '">Resend activation</a>?', 'frmreg' ));
    ...
    */
	public static function activate_url() {
        ...
        if ( ! isset( $_GET['key'] ) || empty( $_GET['key'] ) || ! is_string( $_GET['key'] ) || empty( $_GET['login'] ) || ! is_string( $_GET['login'] ) || ! isset( $_GET['login'] ) ) {
            ...
            FrmRegAppHelper::activation_link_action( $key, $login, $redirect );
        }
        die();
	}

    /*
    * Check if any messages need to be printed on page load
    *
    * @since 1.11
    *
    ...
    */
    public static function print_messages( $content ){
        ...
        if ( $frm_message == 'invalid_login' ) {
            $message = __( 'Invalid username or password. <a href="' . wp_lostpassword_url() . '">Lost your password</a>?', 'frmreg' );
            $class = 'frm_error_style';

        // If pending email confirmation user tries to log in
        } else if ( $frm_message == 'resend_activation' ) {
            $resend_link = FrmRegAppHelper::create_ajax_url( array('action' => 'resend_activation_link', 'user_id' => $_GET['user'] ) );
            $message = __( 'You have not yet confirmed your e-mail address. <a href="' . $resend_link . '">Resend activation</a>?', 'frmreg' );
            $class = 'frm_error_style';
        ...
        // If user is now fully activated
        } else if ( $frm_message == 'complete'  && isset( $_GET['user'] ) && is_numeric( $_GET['user'] ) ) {
        ...
        } else if ( 'login' == $frm_message ) {
            // make sure there is a login form on the page
            if ( ! strpos($content, 'name="pwd"') ) { // check if there is already a password field
        ...
        if ( isset( $message ) ) {
            $content = '<div class="with_frm_style frm_style_default-style"><div class="' . $class . '">' . $message . '</div></div>' . $content;
        }
    ...
    */
    public static function check_for_blank_login($user, $username, $password) {
    	if ( empty($username) || empty($password) ) {
            ...
            $params = array( 'frm_message' => 'login ');
    ...
    */
    public static function redirect_to_login_page( $username = '', $params = array() ) {
        if ( empty( $params ) ) {
            $params = array('frm_message' => 'invalid_login');
        }
        ...
        if ( isset( $settings->login ) && $settings->login ) {
            ...
            $redirect = add_query_arg( $params, get_permalink( $settings->login ) );
            wp_redirect( $redirect );
            exit();
        }
    }
    ...
    */
	public static function resend_activation_notification() {
        if ( isset( $_GET['user_id'] ) && is_numeric( $_GET['user_id'] ) ) {
    		...
    		FrmRegAppHelper::new_user_activation_notification( $user_id, '' );
...
<?php                    
    ...
    */
    ...
    public static function register_widgets() {
        include_once(self::path() .'/widgets/FrmRegLogin.php');
